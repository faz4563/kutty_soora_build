...
try {
	...
	error_log("Orders.php - Failed to load dependencies: " . $e->getMessage());
...
try {
	...
	error_log("Orders.php - JWT validation failed: " . $e->getMessage());
...
if ($action === 'place') {
	...
	if (empty($createdAtInput)) {
		...
		echo json_encode(["success" => false, "error" => "created_at required"]);
		exit;
	}
	...
	if (!$createdAtDate) {
		http_response_code(400);
		echo json_encode(["success" => false, "error" => "Invalid created_at format"]);
		exit;
	}
	...
	try {
        ...
        foreach ($cart_items as $idx => &$item) {
            ...
            // SECURITY: Always use the price from the database. Never trust
            // client-supplied prices or totals - a malicious client could
            // otherwise place an order for a fraction of the real cost.
            $item_price = isset($prod['price']) ? floatval($prod['price']) : 0;
            $item['price'] = $item_price;
            
            // BACKEND CALCULATION (only source of truth)
            $quantity = isset($item['quantity']) ? floatval($item['quantity']) : 0;
            if ($quantity <= 0) {
                error_log("Orders.php - Skipping item with invalid quantity");
                continue;
            }
		...
		// NOTE: no PII (delivery_address / phone) in logs — only ids/numbers.
		error_log("Orders.php - Inserting order: user_id=$user_id, total=$total, order_number=$order_number");
		$stmt = $pdo->prepare("INSERT INTO orders (user_id, order_number, total_amount, subtotal, customer_name, customer_phone, delivery_address, payment_method, payment_status, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$stmt->execute([$user_id, $order_number, $total, $total, $user_name, $phone_number, $delivery_address, 'cash_on_delivery', 'pending', 'pending', $createdAt]);
	...
	try {
		...
		if (!is_array($items)) {
			...
			foreach ($items as &$it) {
				...
				if ($img === '') {
					...
					$it['image_url'] = $protocol . '://' . $host . $scriptDir . '/images/' . $img;
				} else {
		...
		http_response_code(500);
		echo json_encode(["success" => false, "error" => "Failed to create order items. Please try again."]);
		exit;
	}
