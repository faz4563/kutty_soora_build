...
try {
    ...
    // SECURITY: Require JWT authentication AND admin privileges for ALL
    // requests, including GET. Previously GET requests bypassed auth, which
    // let any visitor read or modify admin-only data.
    try {
...
// Build image base URL from the request so admin responses work in both
// local dev and production (previously hardcoded to kuttysoora.com).
...
switch ($method) {
    ...
    case 'POST':
        ...
        try {
            ...
            try {
                ...
                // Field doesn't exist; will be skipped below
            }
            ...
            error_log("Admin Products - Failed to create product: " . $e->getMessage());
            echo json_encode(["error" => "Failed to create product. Please try again."]);
        }
        break;
    ...
    case 'PUT':
        ...
        try {
            ...
            http_response_code(500);
            error_log("Admin Products - Failed to update product: " . $e->getMessage());
            echo json_encode([
    ...
    case 'DELETE':
        ...
        try {
            ...
            http_response_code(500);
            error_log("Admin Products - Failed to delete product: " . $e->getMessage());
            echo json_encode(["error" => "Failed to delete product. Please try again."]);
        }
        break;
